...
export const treinosAvançados: Treino[] = [
  {
    id: "seed-upper-a",
...
export function seedTreinosAvancados(): { added: number; total: number } {
  if (typeof window === "undefined")
    return { added: 0, total: treinosAvançados.length };
  const existing = storage.getAll();
  const existingIds = new Set(existing.map((t) => t.id));
  let added = 0;
  for (const t of treinosAvançados) {
  ...
  return { added, total: treinosAvançados.length };
}
